  ...
  <div>
    ...
    >
      ...
      <b-carousel-slide :img-src="'img/'+dataMonitor.imagen">
        <table style="margin-top: -3rem !important;">
          <tr>
            <th colspan="2"><h2 class="titulo">{{dataMonitor.slug}}</h2></th>
          </tr>
          <tr><td></td><td><h2 class="ref">{{dataMonitor.moneda}}</h2></td></tr> <!-- {{dataMonitor.moneda}} -->
          ...
          <tr>
            <h4 class="producto mt-2">Los productos mostrados no incluyen I.V.A.</h4>
          </tr>
        </table>
      </b-carousel-slide>
      <b-carousel-slide :img-src="'img/'+dataMonitor.imagen">
      ...
      <b-carousel-slide :img-src="'img/'+dataMonitor.imagen">
        <table style="margin-top: -3rem !important;">
          ...
          <tr><td></td><td><h2 class="ref">{{dataMonitor.moneda}}</h2></td></tr>          
          <tr v-for="p in dataMonitor.productos.p3" :key="p.id">
            <td>
              <div class="row ml--4">
                <h2 class="producto">{{p.descripcion}}</h2>
              </div>
            ...
            <td>
              <h1>
                <b-button class="precio" variant="secondary">
                  {{(p.PrecioOferta)?((p.PrecioOferta>0)?p.PrecioOferta:((p.precioDetal)*p.valor)/100):((((p.precioDetal)*p.valor)/100)+(p.precioDetal)).toFixed(2) | moneda}} <!--?p.PrecioOferta:p.precioDetal):p.precioDetal --> <!-- (p.PrecioOferta*dataTasa):(p.precioDetal*dataTasa).toFixed(2)):(p.precioDetal*dataTasa).toFixed(2) -->
                  <b-badge v-if="(p.PrecioOferta)?p.PrecioOferta>0:false" pill variant="danger t--6 ml--3 l-6 h4">OFERTA<i class="fas fa-tag"></i></b-badge>
                </b-button>
              </h1>
            </td>
...
export default {
    ...
    methods: {
      ...
      async loadData(){
        await axios.get(this.$gate.getApiUrl()+"monitor/carniceria/pollo").then(({ data }) => {
