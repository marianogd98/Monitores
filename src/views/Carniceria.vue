<template>
  <div>
    <b-carousel v-if="dataMonitor.length!=0"
      ...
      img-width="1024"
      img-height="480"
      style="text-shadow: 1px 1px 2px #333; padding-top: 0px !important;"
    ...
    >
      ...
      <b-carousel-slide :img-src="'img/'+dataMonitor.imagen">
        <table style="margin-top: -3rem !important;">
          ...
          <tr><td></td><td><h2 class="ref">{{dataMonitor.moneda}}</h2></td></tr>          
          <tr class="mt--3" v-for="p in dataMonitor.productos.p3" :key="p.id">
            <td>
              <div class="row ml--4">
                <h2 class="producto">{{p.descripcion}}</h2>
              </div>
...
export default {
    ...
    methods: {
      ...
      async loadData(){
        await axios.get(this.$gate.getApiUrl()+"monitor/carniceria").then(({ data }) => {
...
@font-face {
  ...
  src: url('../../public/fonts/Philosopher-Italic.ttf');
}
